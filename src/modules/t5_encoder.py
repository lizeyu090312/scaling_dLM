...
def _restore_t5_legacy_3d_mask(_module, args, kwargs):
    attention_mask = kwargs.get("attention_mask")
    if attention_mask is not None and attention_mask.dim() == 5 and attention_mask.shape[1] == 1 and attention_mask.shape[2] == 1:
        kwargs["attention_mask"] = attention_mask.squeeze(2)
    return args, kwargs


def _register_t5_legacy_3d_mask_hooks(model: nn.Module) -> None:
    for block in model.encoder.block:
        block.layer[0].register_forward_pre_hook(_restore_t5_legacy_3d_mask, with_kwargs=True,)
