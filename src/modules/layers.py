...
class FinalLayer(nn.Module):
    """The final layer of ELF, with an optional independent REG head."""

    def __init__(self, hidden_size: int, patch_size: int, out_channels: int, reg_out_channels: Optional[int] = None):
        super().__init__()
        ...
        self.reg_linear = _make_linear(hidden_size, reg_out_channels, bias=True, kernel_init=ZERO_INIT, bias_init=ZERO_INIT,) if reg_out_channels is not None else None

    def forward(self, x: torch.Tensor):
        x = self.norm_final(x)
        if self.reg_linear is None:
            return self.linear(x)
        return self.linear(x[:, 1:]), self.reg_linear(x[:, :1])
