...
# ============================================
# MMLU answer accuracy
# ============================================
_MMLU_ANSWER_RE = re.compile(r"^\s*([A-Da-d])\s*$")
_MMLU_RATIONALE_EXACT_RE = re.compile(r"(?m)^Final answer: ([A-D])$")
_MMLU_RATIONALE_PERMISSIVE_RE = re.compile(
    r"final\s+answer\s*:\s*([A-D])\b", re.IGNORECASE,
)
...
def parse_mmlu_answer(text):
    if text is None:
        return None
    match = _MMLU_ANSWER_RE.match(str(text))
    if match is None:
        return None
    return match.group(1).upper()


def compute_mmlu_accuracy(hypotheses, references):
    ...
    invalid = 0
    for hyp, ref in zip(hypotheses, references):
        pred = parse_mmlu_answer(hyp)
        target = parse_mmlu_answer(ref)
        if target is None:
            raise ValueError(f"Invalid MMLU reference answer: {ref!r}")
        if pred is None:
            invalid += 1
        elif pred == target:
            correct += 1
    accuracy = 100.0 * correct / total if total else 0.0
    return {"mmlu_accuracy": accuracy, "mmlu_correct": correct, "mmlu_total": total, "mmlu_invalid": invalid,}


def parse_mmlu_rationale_answer_exact(text):
    if text is None:
        return None
    text = str(text).strip()
    if text.count("Final answer:") != 1:
        return None
    matches = list(_MMLU_RATIONALE_EXACT_RE.finditer(text))
    if len(matches) != 1 or matches[0].end() != len(text):
        return None
    return matches[0].group(1)


def parse_mmlu_rationale_answer_permissive(text):
    if text is None:
        return None
    matches = _MMLU_RATIONALE_PERMISSIVE_RE.findall(str(text))
    return matches[-1].upper() if matches else None


def compute_mmlu_rationale_accuracy(hypotheses, references):
    if len(hypotheses) != len(references):
        raise ValueError("hypotheses and references must have the same length")
    ...
    permissive_invalid = 0
    for hypothesis, reference in zip(hypotheses, references):
        if not isinstance(reference, str) or reference not in "ABCD" or len(reference) != 1:
            raise ValueError(f"Invalid rationale-MMLU reference answer: {reference!r}")
        exact = parse_mmlu_rationale_answer_exact(hypothesis)
        permissive = parse_mmlu_rationale_answer_permissive(hypothesis)
        if exact is None:
            exact_invalid += 1
        elif exact == reference:
            exact_correct += 1
        if permissive is None:
            permissive_invalid += 1
        elif permissive == reference:
            permissive_correct += 1

    return {
        "mmlu_rationale_exact_accuracy": (
            100.0 * exact_correct / total if total else 0.0
        ),
        "mmlu_rationale_exact_correct": exact_correct,
        "mmlu_rationale_exact_total": total,
        "mmlu_rationale_exact_invalid": exact_invalid,
        "mmlu_rationale_permissive_accuracy": (
            100.0 * permissive_correct / total if total else 0.0
        ),
        "mmlu_rationale_permissive_correct": permissive_correct,
        "mmlu_rationale_permissive_total": total,
        "mmlu_rationale_permissive_invalid": permissive_invalid,
    }
...
# ============================================
_GSM8K_NUMBER_PATTERN = (
    r"[-+]?(?:\d[\d,]*/\d[\d,]*|\d[\d,]*(?:\.\d+)?|\.\d+)"
)
_GSM8K_ANSWER_RE = re.compile(rf"#### ({_GSM8K_NUMBER_PATTERN})(?![\d,./])")
_GSM8K_PERMISSIVE_NUMBER_RE = re.compile(_GSM8K_NUMBER_PATTERN)
_GSM8K_PERMISSIVE_MARKER_RE = re.compile(
    r"####|\bthe answer is\b:?|\banswer:|\banswer is\b", re.IGNORECASE,
)
...
def parse_gsm8k_answer(text):
    ...
    if marker_start < 0:
        return None
    match = _GSM8K_ANSWER_RE.match(text, marker_start)
    return None if match is None else match.group(1).strip().replace(",", "")


def _normalize_gsm8k_number(value):
...
def parse_gsm8k_permissive_answer(text):
    if not text:
        return None
    text = str(text)
    markers = list(_GSM8K_PERMISSIVE_MARKER_RE.finditer(text))
    if markers:
        marker = markers[-1]
        match = _GSM8K_PERMISSIVE_NUMBER_RE.search(text, marker.end())
        return None if match is None else _normalize_gsm8k_number(match.group(0))
    numbers = _GSM8K_PERMISSIVE_NUMBER_RE.findall(text)
    return _normalize_gsm8k_number(numbers[-1]) if numbers else None


def compute_gsm8k_accuracy(
    hypotheses, references, *, eos_emitted=None, response_lengths=None,
):
    if len(hypotheses) != len(references):
        raise ValueError("hypotheses and references must have the same length")
    total = len(hypotheses)
    ...
    permissive_invalid = 0
    for hyp, ref in zip(hypotheses, references):
        pred = parse_gsm8k_answer(hyp)
        target = parse_gsm8k_answer(ref)
        if target is None:
            raise ValueError(f"Invalid GSM8K reference answer: {ref!r}")
        if pred is None:
            invalid += 1
        elif pred == target:
            correct += 1
        permissive_pred = parse_gsm8k_permissive_answer(hyp)
        if permissive_pred is None:
            permissive_invalid += 1
        elif permissive_pred == _normalize_gsm8k_number(target):
            permissive_correct += 1
    result = {
        "gsm8k_accuracy": 100.0 * correct / total if total else 0.0,
        "gsm8k_correct": correct,
        "gsm8k_total": total,
        "gsm8k_invalid": invalid,
        "gsm8k_permissive_accuracy": (
            100.0 * permissive_correct / total if total else 0.0
        ),
        "gsm8k_permissive_correct": permissive_correct,
        "gsm8k_permissive_total": total,
        "gsm8k_permissive_invalid": permissive_invalid,
    }
    if eos_emitted is not None:
        if len(eos_emitted) != total:
            raise ValueError("eos_emitted must match hypotheses")
        result["gsm8k_eos_rate"] = (
            100.0 * sum(bool(value) for value in eos_emitted) / total
            if total else 0.0
        )
    if response_lengths is not None:
        if len(response_lengths) != total:
            raise ValueError("response_lengths must match hypotheses")
        lengths = np.asarray(response_lengths, dtype=np.float64)
        result.update({
            "gsm8k_response_length_mean": float(lengths.mean()) if total else 0.0,
            "gsm8k_response_length_median": float(np.median(lengths)) if total else 0.0,
            "gsm8k_response_length_p95": float(np.percentile(lengths, 95)) if total else 0.0,
        })
    return result


# ============================================
# MATH symbolic answer accuracy
# ============================================
def compute_math_accuracy(
    ...
    try:
        ...
        raise ImportError(
            "math_accuracy requires math-verify[antlr4_13_2]==0.9.0"
        ) from error

    gold_config = (LatexExtractionConfig(boxed_match_priority=0),)
    prediction_config = (
    ...
    for hypothesis, reference in zip(hypotheses, references):
        if reference is None or not str(reference).strip():
            raise ValueError(f"Invalid MATH reference answer: {reference!r}")
        reference_for_parse = str(reference).strip().replace("\n", " ")
        gold = parse(
            f"${reference_for_parse}$", extraction_config=gold_config,
        )
        if not gold:
            raise ValueError(f"Invalid MATH reference answer: {reference!r}")

        prediction_for_parse = (
            "" if hypothesis is None else str(hypothesis).replace("\n", " ")
        )
        prediction = parse(prediction_for_parse, extraction_config=prediction_config)
        if not prediction:
            invalid += 1
        elif verify(gold, prediction):
            correct += 1

    result = {
        "math_accuracy": 100.0 * correct / total if total else 0.0,
        "math_correct": correct,
        "math_total": total,
        "math_invalid": invalid,
    }
    if eos_emitted is not None:
        if len(eos_emitted) != total:
            raise ValueError("eos_emitted must match hypotheses")
        result["math_eos_rate"] = (
            100.0 * sum(bool(value) for value in eos_emitted) / total
            if total else 0.0
        )
    if response_lengths is not None:
        ...
        lengths = np.asarray(response_lengths, dtype=np.float64)
        result.update({
            "math_response_length_mean": float(lengths.mean()) if total else 0.0,
            "math_response_length_median": float(np.median(lengths)) if total else 0.0,
            "math_response_length_p95": (
                float(np.percentile(lengths, 95)) if total else 0.0
            ),
        })
    return result
