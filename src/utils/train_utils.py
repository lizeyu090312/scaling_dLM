...
def stable_train_seed(base_seed: int, rank: int, step: int) -> int:
    data = f"train|{int(base_seed)}|{int(rank)}|{int(step)}".encode("utf-8")
...
@dataclass
class TrainState:
    """Lightweight container for the trainable model and its EMA shadows."""

    model: nn.Module
    optimizer: Optimizer
    lr_scheduler: Any = None
    ema_params1: Dict[str, Any] = field(default_factory=dict)
    step: int = 0
    epoch: int = 0
    sample_offset_in_epoch: Optional[int] = 0
    group_by_length_active: bool = False
    ...
    def replace(self, **kwargs) -> "TrainState":
        new = TrainState(
            ...
            sample_offset_in_epoch=self.sample_offset_in_epoch,
            group_by_length_active=self.group_by_length_active,
    ...
    @staticmethod
    def init_emas(model: nn.Module, decays) -> Dict[str, Dict[str, torch.Tensor]]:
        return {ema_decay_key(decay): TrainState.init_ema(model) for decay in decays}


def ema_decay_key(decay: float) -> str:
    return format(float(decay), ".12g")


def is_multi_ema_state(ema_params) -> bool:
    return bool(ema_params) and all(isinstance(v, Mapping) for v in ema_params.values())
...
# EMA update
# ============================================
def ema_decay_for_update(target_decay: float, optimizer_step: int, warmup_updates: int) -> float:
    """Return EDM-style EMA decay with linear warmup in half-life."""
    if not (0.0 < target_decay < 1.0): raise ValueError("target_decay must be in (0, 1)")
    if warmup_updates < 0: raise ValueError("warmup_updates must be non-negative")
    if warmup_updates == 0: return target_decay
    if optimizer_step <= 0: raise ValueError("optimizer_step must be positive during EMA warmup")

    target_halflife = math.log(0.5) / math.log(target_decay)
    progress = min(optimizer_step / warmup_updates, 1.0)
    current_halflife = target_halflife * progress
    return 0.5 ** (1.0 / current_halflife)
